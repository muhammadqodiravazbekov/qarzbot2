...
<body class="bg-[#f8fafc] text-[#0f172a] font-sans antialiased pb-24 selection:bg-indigo-50">
    ...
    <div class="max-w-md mx-auto p-4 space-y-4">
        ...
        <div class="space-y-2">
            ...
            <div id="records-container" class="space-y-2 hidden"></div>
        </div>
    </div>
    ...
    <div id="profile-drawer" class="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-end justify-center hidden opacity-0 transition-opacity duration-200">
        <div class="bg-white w-full max-w-md rounded-t-3xl p-5 space-y-4 shadow-2xl transform translate-y-full transition-transform duration-200 max-h-[92vh] overflow-y-auto no-scrollbar">
            ...
            <div class="space-y-2">
                ...
                <div id="drawer-history-container" class="space-y-1.5 max-h-40 overflow-y-auto no-scrollbar"></div>
            </div>
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'adduser':
        try:
            ...
            context.user_data['action'] = 'add_user_role'
...
# ---------- Seamless Synchronous Webhook Interface Route ----------
...
def telegram_webhook_endpoint():
    try:
        ...
        logging.error(f"Webhook Execution Error Context: {e}")
    return 'OK', 200

# ---------- System Orchestration Initialization Lifecycle ----------
async def initialize_application_lifecycle():
    init_db()
    await bot_app.initialize()
    await bot_app.start()
    
    app_host = os.environ.get('RENDER_EXTERNAL_HOSTNAME', 'qarzbot2-1.onrender.com')
    webhook_url = f"https://{app_host}/{BOT_TOKEN}"
    await bot_app.bot.set_webhook(url=webhook_url)
    logging.info(f"Webhook bound securely to infrastructure destination: {webhook_url}")

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    
    # Initialize components safely inside event loops before kicking off WSGI interfaces
    asyncio.run(initialize_application_lifecycle())
