...
def get_current_time(): return datetime.now(UZB_TZ)

# ---------- Flask Web Server ----------
flask_app = Flask(__name__)
@flask_app.route('/')
@flask_app.route('/health')
def health(): return jsonify({"status": "alive", "message": "Bot is running optimally!"}), 200
...
def get_db(commit=False):
    """Context manager for clean DB connections using dictionary cursors."""
...
async def notify_group(context: ContextTypes.DEFAULT_TYPE, action: str, customer: str, amount: float, seller: str, note: str = "", old_bal: float = None, new_bal: float = None):
    ...
    msg = f"📢 **{action}**\n\n👤 Мижоз: {customer}\n💰 Сумма: {amount:,.2f} сўм\n"
    if old_bal is not None and new_bal is not None: msg += f"📊 Баланс: {old_bal:,.2f} ➡️ {new_bal:,.2f} сўм\n"
    ...
    except Exception as e: logging.error(f"Group notification failed: {e}")
...
async def add_notes_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    if update.callback_query.data == "skip_notes": return await process_new_debt_save(context, update, "")
...
async def exist_search_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): return await search_and_select(update, context, EXIST_AMOUNT, "exist")
async def pay_search_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): return await search_and_select(update, context, PAY_AMOUNT, "pay")
...
async def select_debt_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "exist":
        msg, kb, state = f"👤 **{debt['customer_name']}**\n📊 Қарз: {debt['remaining_balance']:,.2f} сўм\n\n💰 **Қўшиладиган сумма:**", cancel_inline_keyboard(), EXIST_AMOUNT
    else:
        msg = f"👤 **{debt['customer_name']}**\n💸 Қарз: {debt['remaining_balance']:,.2f} сўм\n\n💵 **Тўлов суммаси:**"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton(f"💰 Тўлиқ ёпиш ({debt['remaining_balance']:,.0f} сўм)", callback_data=f"payfull_{debt['remaining_balance']}_{debt_id}")],
                                   [InlineKeyboardButton("❌ Бекор қилиш", callback_data="cancel_action")]])
        state = PAY_AMOUNT
...
async def exist_amount_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except ValueError: return EXIST_AMOUNT
    context.user_data.clear()
    return ConversationHandler.END
...
async def pay_amount_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: return await process_payment(context, update, float(update.message.text.strip()), context.user_data['selected_debt_id'])
...
async def search_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not debts: await update.message.reply_text("🔍 Топилмади.")
    else:
...
async def send_backup_to_group_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not debts or not BACKUP_GROUP_ID: return await update.message.reply_text("Хато: База бўш ёки гуруҳ йўқ.")
    
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["ID", "Name", "Current Debt", "Date"])
    for d in debts: writer.writerow([d['id'], d['customer_name'], d['remaining_balance'], d['created_at']])
    output.seek(0)
    
    try: await context.bot.send_document(chat_id=int(BACKUP_GROUP_ID), document=io.BytesIO(output.getvalue().encode()), filename="Backup.csv")
    except Exception: await update.message.reply_text("❌ Хатолик.")

# (User Management Handlers omitted for brevity in response, but logic remains intact via get_db)
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.clear(); await update.message.reply_text("🚫 Бекор қилинди.")
    return ConversationHandler.END
...
def run_telegram_bot():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ADD_NAME: [MessageHandler(filters.TEXT, add_name_handler)],
            ADD_AMOUNT: [MessageHandler(filters.TEXT, add_amount_handler)],
            ADD_NOTES: [MessageHandler(filters.TEXT, add_notes_text_handler), CallbackQueryHandler(add_notes_callback_handler, pattern="^skip_notes$")],
            EXIST_SEARCH: [MessageHandler(filters.TEXT, exist_search_handler)],
            EXIST_AMOUNT: [MessageHandler(filters.TEXT, exist_amount_handler)],
            PAY_SEARCH: [MessageHandler(filters.TEXT, pay_search_handler)],
            PAY_AMOUNT: [MessageHandler(filters.TEXT, pay_amount_text_handler), CallbackQueryHandler(pay_full_callback_handler, pattern="^payfull_")],
            SEARCH_QUERY: [MessageHandler(filters.TEXT, search_query_handler)]
        },
        fallbacks=[CommandHandler("cancel", cancel), CallbackQueryHandler(cancel_callback, pattern="^cancel_action$"), CallbackQueryHandler(select_debt_callback, pattern="^(exist|pay)_")]
    )
