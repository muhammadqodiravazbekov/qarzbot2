...
# ========== КОНФИГУРАЦИЯ ==========
BOT_TOKEN = "8912188749:AAGCVslE1Ry9kHhOMnpb7ejV_eIF6O37x4w"
...
def normalize_text(text: str) -> str:
    """Кирилл ва Лотин ҳарфларини бир хил кўринишга келтиради (case-insensitive)"""
    if not text:
        return ""
    # Кирилл-Лотин транслитерацияси (асосий ҳарфлар)
    cyrillic_to_latin = {
    ...
    # Кириллни Лотинга ўтказиш
    normalized = text.lower()
    for cyr, lat in cyrillic_to_latin.items():
        normalized = normalized.replace(cyr, lat)
    # Акцентларни олиб ташлаш (umlaut, accent)
    normalized = unicodedata.normalize('NFKD', normalized).encode('ASCII', 'ignore').decode('ASCII')
    # Фақат ҳарфлар ва рақамларни қолдириш
...
def init_db():
    ...
    # Add normalized name column if not exists
...
# ========== МАЪЛУМОТЛАР БАЗАСИ ФУНКЦИЯЛАРИ ==========
def get_db():
...
def search_debts(query: str) -> List[Dict]:
    """Қидирув: телефон ёки нормаллаштирилган ном бўйича (тез, ҳарф сезгирлиги йўқ)"""
    norm_query = normalize_text(query)
    conn = get_db()
    cursor = conn.cursor()
    # Телефон рақамига ёки нормаллаштирилган номга қараб қидиради
    cursor.execute("""
...
def main():
    ...
    print("Бот ишга тушди (оптимизацияланган қидирув билан) ...")
    app.run_polling()
